-- Make verify_cash_payment SECURITY INVOKER and grant necessary RLS policies

-- 1. Add UPDATE policy for admins on accounts
CREATE POLICY "Admins can update accounts" ON public.accounts
  FOR UPDATE
  TO authenticated
  USING (EXISTS (SELECT 1 FROM public.admin_users WHERE user_id = auth.uid()));

-- 2. Add INSERT policy for admins on cash_payment_verifications
CREATE POLICY "Admins can insert cash payment verifications" ON public.cash_payment_verifications
  FOR INSERT
  TO authenticated
  WITH CHECK (EXISTS (SELECT 1 FROM public.admin_users WHERE user_id = auth.uid()));

-- 3. Change function to SECURITY INVOKER
